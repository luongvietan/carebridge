...
function StarRating({ compact = false }: { compact?: boolean }) {
    ...
    return (
      <div className="flex flex-col items-end gap-1">
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-sm font-semibold text-[#1e5a33]">5.0</span>
          <div className="flex gap-0.5" aria-label="5 out of 5 stars">
            {Array.from({ length: 5 }).map((_, i) => (
              <Icon key={i} icon={StarIcon} size={12} color="#1e5a33" strokeWidth={1.75} />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="flex gap-0.5" aria-label="5 out of 5 stars">
        {Array.from({ length: 5 }).map((_, i) => (
          <Icon key={i} icon={StarIcon} size={14} color="#2e7d32" strokeWidth={1.75} />
        ))}
      </div>
    </div>
  );
