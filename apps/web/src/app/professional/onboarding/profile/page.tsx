...
export default async function ProfilePage() {
  ...
  const { data: current } = user
    ? await supabase
        .from("professionals")
        .select(
          "id, professional_role_id, date_of_birth, address_line1, address_line2, city, postcode, national_insurance_no, professional_summary, travel_distance_km, has_driving_licence, has_vehicle",
        )
        .eq("user_id", user.id)
        .maybeSingle()
    : { data: null };
